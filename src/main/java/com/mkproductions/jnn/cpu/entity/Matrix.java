...
public class Matrix {
    ...
    public static Matrix matrixMultiplication(Matrix matrix1, Matrix matrix2) {
        if (matrix1.getColumnCount() != matrix2.getRowCount()) {
            ...
            throw new IllegalArgumentException(STR."Matrix multiplication not possible. Cause\{matrix1.getColumnCount()} != \{matrix2.getRowCount()}");
        }
    ...
    public static Matrix convolute(Matrix matrix, Matrix filter) {
        // Input validation
        if (filter.getRowCount() > matrix.getRowCount() || filter.getColumnCount() > matrix.getColumnCount()) {
            throw new IllegalArgumentException(
                    "Filter dimensions (" + filter.getRowCount() + "," + filter.getColumnCount() + " cannot exceed image dimensions(" + matrix.getRowCount() + "," + matrix.getColumnCount() + ")");
        }

        // Calculating the output shape.
        int resultRowCount = matrix.getRowCount() - filter.getRowCount() + 1;
    ...
    public static Matrix createFromDiagonalToColumnMatrix(Matrix result) {
        double[] results = new double[result.getRowCount()];
    ...
    public static Matrix transpose(Matrix matrix) {
        Matrix result = new Matrix(matrix.getColumnCount(), matrix.getRowCount());
        for (int a = 0; a < result.getRowCount(); a++)
            for (int b = 0; b < result.getColumnCount(); b++)
    ...
    public static Matrix elementWiseMultiply(Matrix matrix1, Matrix matrix2) {
        if (matrix1.getRowCount() != matrix2.getRowCount() || matrix1.getColumnCount() != matrix2.getColumnCount()) {
            System.err.println(STR."Matrix1: (\{matrix1.getRowCount()}, \{matrix1.getColumnCount()}");
            System.err.println(STR."Matrix2: (\{matrix2.getRowCount()}, \{matrix2.getColumnCount()}");
    ...
    public ArrayList<Double> getRowCount(int rowIndex) {
