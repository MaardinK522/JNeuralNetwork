...
public enum ActivationFunction {
    ...
    LINEAR("linear", (a, b, x) -> x, (a, b, y) -> 1), // Linear activation function with derivative.
    TAN_H("tan_h", (a, b, x) -> (Math.exp(x) - Math.exp(-x)) / (Math.exp(x) + Math.exp(-x)), (a, b, y) -> 1 - (y * y)), // Hyper tangent activation function with derivative.
    SOFTMAX("softmax", ((row, column, value) -> 0.0F), (row, column, value) -> 0.0F), // Soft max activation without function or derivative.
    ;
